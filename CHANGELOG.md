...
Minor release with four threads: **a smaller always-on context budget on every session start** (the install-seeded rule entries shrink from five entries totalling about 6.8 KB to four totalling about 2.4 KB, the rules README stops loading as a rule, a template leak that seeded every fresh memory index with 2.5 KB of scaffolding is closed, and a shipped monitor now measures every loaded instruction surface per session instead of leaving it to memory), **plan arming scoped to the spoke that armed it** (the active-plan pointer moves under each project's own binder directory, so two projects arm concurrently and a session receives only the spec of the plan armed in its own launch directory), **one task-status vocabulary across the whole corpus** (a migration converges eight legacy task-status spellings onto the five declared values, so every reader agrees on which tasks are settled), and **trustworthy dangling-link findings** (two whole classes of legitimate wikilink stop being reported as broken). **Two automatic upgrade-time migrations ship in this release** — one over your plans corpus (task statuses) and one over your rules directory (the shortened rule seeds) — both idempotent and both recoverable; the [v1.18.0 release notes](docs/release-notes-v1.18.0.md) say exactly what each does and what it will never do.

### Added

- **Context-budget monitor.** The hook on the instructions-loaded event now records the size of every instruction file the harness loads, the session-end hook counts hook outputs that overran the harness's 10,000-character cap and were spilled to disk, and a new librarian capability, `context-budget-index`, renders those measurements against declared per-surface thresholds — so the cost of the always-on context is a number in a report, not a guess.
- **Rule compaction for hook-backed rules.** A rule whose enforcement already lives in a deterministic hook can be compacted to a short stub that names the hook: a new librarian capability, `rules-compact`, performs the compaction, the rules schema gains an `enforced_by` field to record it, and any rule lacking the keep and why markers the compaction needs is refused rather than emptied. The memory-globalize capability gains `--target` so a promotion can be aimed at a chosen rules directory.
- **Per-spoke plan arming.** The armed-plan pointer now lives at `_projects/<spoke>/.active-plan` inside each project's binder directory, resolved through one shared library that every reader uses. Two projects arm at the same time without contention, the spec-authority injection lands only in sessions launched from the spoke where the plan is armed, and a session in a spoke with nothing armed receives nothing — the largest hook payload brain-stem ships stops loading into every session on the machine. The old corpus-wide pointer is still honoured as a read-only fallback; nothing writes it any more.
- **Task-status migration.** Migration `0009-task-status-vocabulary` maps the eight legacy task-status spellings (`complete`, `completed`, `closed`, `pending`, `planned`, `needs-revision`, `deferred`, `retired`) onto the five declared values, task axis only, leaving every other value byte-unchanged; each manifest it changes is first copied to a `.pre-0009` sidecar beside it.
- **Rules-refresh migration.** Migration `0010-class-a-rules-refresh` delivers the shortened rule seeds to an existing install, which the seeder alone cannot do because it preserves any file that already exists. It replaces a seed only when the file still matches a body this project itself wrote, retires the rules README and the bounded-spoke rule on the same recognition, and leaves any adopter-edited file exactly as it is.
- **claude-mem guidance.** Adopters running the optional claude-mem plugin are told — in the onboarder's external-setup step, in the onboarder skill and in the README — how to turn off the context index the plugin injects at session start, and why shrinking it to sit just under the cap is worse than either extreme.

### Fixed

- **Fresh memory indexes no longer inherit template scaffolding.** The memory-index template documented itself in comment blocks the harness does not strip the way the template assumed, so every freshly seeded index carried 2,529 bytes over 50 lines of scaffolding against an intended 55 bytes over 7 lines. Two hunks close both seams, and the template now carries a short visible note naming the three memory types and where the schema lives.
- **The session chronicle pointer lands on template-seeded indexes.** The session-end hook that refreshes the chronicle pointer in the memory index chose its branch with a substring test and did the replacement with an exact-line test; on an index seeded from the template the two disagreed, the file was rewritten unchanged, and the pointer never landed. One shared whole-line predicate now drives both, and the chronicle indexer locates its sentinels by anchored line so it can only touch a live block.
- **Rotated chronicle archives stop accumulating dead index lines.** The memory-consolidation orphan check indexed every rotated chronicle archive as a new entry with an empty hook, one per rotation, forever. The archives are now exempt from that check.
- **The memory-index cap is stated in the unit the harness measures.** The load limit is 25,000 UTF-16 code units of stripped text, not 25,600 raw bytes, and the harness appends a visible warning when it cuts — the governance mandate, the write-time guard and the template now say so, and the warning band no longer sits inside the cut window.
- **Dangling-link findings are trustworthy again.** The write-time wikilink check reported two whole classes of legitimate link as broken: a `[[name.ext]]` link to a non-markdown file, and a `[[Surface/...]]` link whose target is reached through a vault-root symlink. Both resolve now.
- **One terminal split for tasks.** The project-situating reader that decides whether a task is settled keys on the declared terminal pair (`done`, `cut`) instead of a private broader set.

### Changed

- **Smaller always-on rule seeds.** The four install-seeded rule entries are rewritten to about a third of their size; the rules README is no longer seeded into the rules directory (the harness loads every markdown file there at launch — the canonical copy ships as a template instead, and the write-time rules advisory points there); and the bounded-spoke rule is retired because the session-start home-launch hook already delivers its guidance at exactly the moment it applies.
- **Shorter skill descriptions.** Seven shipped skill descriptions are trimmed by about 400 bytes in total, shrinking the skill roster the harness renders into every session.
- **Guard messages point at rules, not headings.** The decision-quality guard no longer cites CLAUDE.md section headings that no install path produces; each message keeps its operative instruction inline.

## [v1.17.0]

Minor release with four threads: **one-command upgrade** (a single idempotent entry point detects your installed version and setup shape, previews the plan, and applies it — no more per-release flag recipes), **a live install that cannot silently drift** (editing a brain-stem-managed file in place now surfaces an in-flight advisory, and a standing daily census hashes every managed file against the manifest so any divergence — whatever caused it — surfaces within one scheduled cycle), **convergence of live improvements into the product** (renderer improvements that existed only as live patches are now shipped, in lockstep with the schema and every file-creation surface), and **one declaration instead of many** (corpus-walking capabilities consume one machine-readable surface roster, every task-status reader keys on the one declared vocabulary, and an overloaded advisory label family is renamed so each label resolves uniquely). **No new migrations ship in this release:** upgrading is a file sync and needs nothing from you by hand. See the [v1.17.0 release notes](docs/release-notes-v1.17.0.md).

### Added

- **One-command upgrade.** `install.sh --upgrade` asserts an existing install, detects its version and setup shape, auto-resolves the install home, and previews exactly what an apply will do; `install.sh --upgrade --apply` performs it. The per-release bespoke command sequence — a different flag set every cycle, reconstructed by hand — is retired. Every line the installer prints during install and upgrade was audited in the same pass: stale advice removed, next-steps made actionable, diagnostics reworded to current fact.
- **Managed-file write advisory.** Writing to a file that brain-stem manages in your live install (a shipped hook, skill, or governance file) now draws an in-flight advisory naming the managed surface and the sanctioned route (change the product, ship it, upgrade). An explicit environment hatch acknowledges a deliberate exception, and every fire is logged to a telemetry ledger so the posture can be tightened on evidence rather than assumption.
- **Standing managed-surface census.** A daily scheduled census hashes every managed file against the shipped manifest and reports divergence as findings — so a drift that arrived by any vector (a cron job, an outside tool, a hand edit that predates the advisory) is surfaced within one cycle instead of accumulating unseen.
- **A machine-readable surface roster.** The librarian's corpus-walking capabilities — link checking, enumeration, placement validation — consume one declared roster of live corpus roots and retired surfaces instead of each hand-coding its own root set. Link checking also models editor-faithful suffix resolution and reports findings under scope-honest headlines.
- **Declared traceability-matrix mapping.** The plan manifest schema now declares the traceability-matrix filename mapping, so writers and validators read one declaration instead of each restating the convention.

### Fixed

- **Renderer output matches the schema everywhere.** The task-ledger and traceability-matrix renderers now emit the full frontmatter cohort on the files they write — improvements that previously existed only as unshipped patches on one live install are shipped product, landed in lockstep with the schema and the write-time gates so no surface disagrees.
- **Overlay collision checking covers every entity class.** Entries in array-valued slots and entities with underscore-prefixed names were silently skipped by the collision walk — an adopter override of one of those succeeded without the override reason that object-shaped, plainly-named entities already require. The walk now gates them all, entity identity is registry-driven, and the install lane probes for pre-existing violations at apply time with a warning rather than a hard stop.
- **Research artifacts survive a same-session close.** A plan reaching terminal status in the very session that would first declare its research artifacts had those declarations suppressed — the close-out chain stamped the terminal state before deriving the declaration. The chain now declares first, and a closed-scope warning lands on a durable surface instead of vanishing with the session.
- **The crash-guard checker understands quotes.** The checker that screens shell surfaces for unguarded command substitutions treated a quoted or escaped parenthesis as span syntax, producing false positives that were papered over with exemption markers. Quoted parentheses are now data; the paper-over markers are retired.
- **Binder projection surfaces are ruled and self-consistent.** The per-project research symlink farm — a projection with no remaining consumer — is removed across every shipped surface, and decision-class artifacts route to the decision log by declared kind instead of by folder accident.
- **The advisory-fixture corpus is drained to zero.** Every standing advisory-census failure was repaired (fixtures re-anchored to durable facts) or ruled (the one real product defect fixed), and the census now fires once per release ceremony as a standing recurrence layer, so fixture rot surfaces at most one release after it lands.

### Changed

- **Project-container tags.** Tag inference for files under the projects root now emits the project-scoped container form (`#project/<name>`), aligning emitted tags with the foundation's container dimension.
- **Every file-creation surface emits the full frontmatter cohort.** Plan scaffolding, graduation, and sub-plan minting all emit the same complete key set the renderers and gates expect — a file is born conformant instead of healed later.
- **One task-status vocabulary.** Every shipped reader of a task-status field keys on the single declared vocabulary in the plan manifest schema; the private near-duplicate status sets that had accumulated in individual readers are deleted, and a census fixture keeps a fourth vocabulary from ever appearing.
- **Advisory labels resolve uniquely.** A write-time advisory family that shared its label with two unrelated rule families is renamed to its registered identity, so tracing any shipped advisory string lands on the rule that actually governs it.
- **Memory hygiene respects dotfiles and declared exemptions.** Editor artifacts and dotfiles are excluded from memory-corpus hygiene scans, and deliberate exemptions are configurable instead of hardcoded.
...
- **Session close is reliable again, end to end.** Four independent defects in the close-out chain are fixed together: the idempotency guard is evaluated at the start of the chain, so a re-entrant close is a no-op instead of a duplicate pass; the chronicle appender recognizes a block it already carries and appends nothing; the project-spoke resolver resolves both endpoints through the real filesystem, so two spellings of one directory can no longer split a project into two identities; and the detached background job that finishes a close is told which session it is closing instead of inferring it from its own working directory — which for a job spawned by a scheduler meant the wrong answer. Together these end the observed failure where a close-out written from the wrong context duplicated or misfiled another project's records.
- **Librarian manifest validation passes on healthy manifests.** The schema described a drift-findings shape no writer ever produced, so a correct, healthy manifest failed validation at every session close. The schema now states the per-scope shape the writers actually produce, the skeleton seeds only the parent namespace, and the close-out receipt line flips from a standing error to ok — with no data migration, because the data was never wrong.
- **Your checkpoint can no longer be silently overwritten.** The write predicate for session checkpoints is owned by a single guard, closing a path where a passive re-write could clobber a checkpoint another step had just written.
- **Hooks and background jobs no longer wait on input that never arrives.** Completing the family started in the previous release: every confirmed stdin-reading site in the shipped hook surface now bounds its read, and the convention is gated so a new unbounded read cannot ship.
- **The install does not invent paths it does not own.** A sweep of the shipped surface removed defaults that pointed into a home directory the install does not control, the vault location is resolved from the recorded configuration instead of being guessed, and an unconfigured vault is refused with a clear message instead of silently adopted. The portability check that catches this class now covers the full shipped roster rather than a founding subset.
- **The cron-health banner reports the lanes you actually have.** The banner derives its lane roster from what is actually registered instead of a fixed list, and knows the difference between lane types — ending false staleness warnings for lanes that were healthy or absent by design.
- **Governance internals describe themselves truthfully.** The write-guard reads the naming rules at the path where they actually live; the project scaffolder no longer stamps document types the allowlist does not carry; and the manifest-recording helper writes the schema shape it declares.

### Changed

- **Manifest text keeps its characters.** Manifest re-serializers now write non-ASCII text as-is instead of escaping it, so quotes, dashes, and accented characters in your plan text survive a round-trip readably.
- **Schema examples are anonymous.** Two schema examples that carried real machine identifiers now carry placeholder ones.
- **Shipped comments resolve better for you.** The unresolvable compound register citations across the shipped comment surface — references only the maintainer's machine could resolve — are reworded to state the durable fact inline. A small residue of older build-era references remains and shrinks release over release; the release pipeline now gates new ones.
- **The release pipeline earns more of your trust.** The pipeline that produces this public tree now fails closed on vocabulary it has never seen (a new provenance-token family is flagged by default rather than silently shipped), measures every strip-family across its variant axes, propagates every changed public file or fails the gate, and verifies its blind reviewers' own tools before trusting their verdicts. Maintainer-side, but it is the machinery your download comes from.

## [v1.14.0]

Minor release — a governance-contract completion, a librarian accuracy pass, a written policy covering the parts of your install you own, and the change with the widest reach here: **the background work brain-stem detaches at session start and end no longer inherits whatever environment the session happened to have.** Previously, a session running against a non-default Claude home could have that detached work resolve — and in one case write into — a different install than the one it belonged to. Three hooks that run at every session start and end are affected, so this reaches every session even though nothing about it is visible day to day. **No migrations ship in this release:** upgrading is a file sync and needs nothing from you by hand. See the [v1.14.0 release notes](docs/release-notes-v1.14.0.md).

### Fixed

- **Detached session-boundary work stays inside the install it belongs to.** brain-stem spawns short background jobs when a session starts and ends. Those jobs inherited the ambient environment, and the resolver that supplies default paths mints a live value in the parent and hands it to every child — so nothing pinned which install a detached job was working on. A session running against a relocated or temporary Claude home could therefore have its background job read the live install's capabilities and configuration instead, and one path made it a write: the memory-consolidation job re-resolved its own memory directory child-side and created it. Every detached spawn point now pins its environment explicitly before handing off, so a background job always operates on the install that spawned it.
- **An overlay that sets a file type to null is now rejected instead of quietly corrupting that type.** The governance overlay merges your customizations over the shipped foundation. A merge cannot delete a key, and the guard that derives accepted file types from the merged result did no subtraction — so writing `null` against a file type produced a type that was still accepted but carried no contract at all, which is worse than leaving it alone. That shape is now rejected with a finding that names it, rather than silently accepted.
- **Every shipped file-type contract is now explicitly accounted for.** The foundation ships thirteen file-type contracts while its type registry names only six types, and nothing recorded how — or whether — the remaining contracts were wired to enforcement. Each contract now carries an explicit disposition in the governance rules: registered to a type, an alias of a registered type, or governed by the file's path with the contract text serving as documentation. This is an accounting change, not new enforcement — no previously-inert check starts firing — but a contract can no longer sit unregistered with nothing even recording that fact.
- **The librarian's checks now do what they describe.** A sweep of the librarian's own contract surfaces found places where its self-description had drifted from its behavior while every gate stayed green. Fixed and added here: the disposition check on session handoffs no longer raises false positives on plan names and inline code spans; a standing parity arm now measures that every vault-walking capability actually reaches through the symlink-composed vault view, so a walk that silently stops at a link boundary is a reported drift rather than an invisible gap; and the tag vocabulary the generators emit is asserted against the registry that declares it, so the two cannot drift apart unnoticed. Two behavior changes ride along: the generators' emitted project tag loses a trailing `s` — it is now `#project/<name>`, where earlier versions emitted the plural form of the same prefix; files generated by earlier versions keep the plural until they are next regenerated — and wikilink targets that live in the memory corpus (your rules directory, per-project memory) are no longer reported as broken links.
- **A background capability no longer stalls waiting on input that never arrives.** Capability jobs launched at a session boundary inherited an open input stream that never reached end-of-file, so a capability that read from input could wait indefinitely instead of finishing.

### Changed

- **The governance overlay's operation set is stated honestly: add and supersede.** Removing something through the overlay was never supported by the merge underneath, and the attempt produced the corrupt shape described above. Removal is expressed through the existing retired-types tombstone, which already refuses writes, rather than through a null the merge cannot honor.
- **Your rules directory and per-project memory are documented as yours, and your edits survive upgrades.** Both are written by shipped capabilities at runtime, so neither is protected against editing — that is deliberate, and this release's notes state it plainly rather than leaving it implied. The rules entries brain-stem seeds are written only when absent, so a rule you author or edit is never overwritten by a later upgrade. The trade-off is stated plainly: because seeding never clobbers, an updated version of a seeded rule reaches only new installs, and existing installs keep the copy they have.

## [v1.13.1]

Patch release — five correctness fixes, headlined by an upgrade-lane repair that matters to anyone who installed v1.13.0: **the v1.13.0 upgrade could fail to deliver that release's own new migrations.** The upgrade preview listed them, the version stamp advanced, but the migrations were never copied or run — so the corpus convergence v1.13.0 promised may not have happened on your install, with no error to tell you. v1.13.1 fixes the delivery lane and **heals bitten installs automatically**: migration selection now runs every migration your install has not yet applied, regardless of how far the version stamp advanced. Upgrading is the only action needed — then confirm with the post-apply self-check in the release notes. One new migration (0008) ships in this release and runs automatically at `install.sh --apply`. See the [v1.13.1 release notes](docs/release-notes-v1.13.1.md).

### Fixed

- **A release's own new migrations are now delivered and run at upgrade — and installs the v1.13.0 defect bit are healed automatically.** The upgrade lane enumerated the migration files to copy from the *previous* release's frozen baseline, which by construction never listed the *new* release's own migrations — so v1.13.0's `--apply` previewed migrations 0005/0006/0007, delivered none, ran none, and advanced the version stamp anyway. Two repairs close both horns of the defect: the lane now enumerates the copy-set from the shipped target manifest, and the runner selects migrations by set-difference against the applied-ids record instead of trusting the version floor — so an install whose stamp already advanced past an unapplied migration picks it up on the next `--apply`. After upgrading, verify with the self-check in the release notes: the `migrations_applied[]` record should include `0005`, `0006`, `0007`, and `0008`.
- **Fresh plans no longer scaffold a permanent duplicate task-list section.** The plan scaffolders emitted a `## Tasks` heading (and, on the sub-plan path, unfilled placeholder blocks) outside the render markers that bound the generated task list, so the first regeneration preserved that block as a footer forever — invisible even to the renderer's own parity check. Scaffolding now delegates the whole region to the renderer — the single writer of that section — and ships an empty marker pair, so a fresh plan renders exactly one task list. Plans scaffolded before the fix keep their footer; the release notes describe the one-time cleanup.
- **The checkpoint safeguard no longer false-fires right after a context compaction.** brain-stem asks for a session checkpoint when a session's context pressure runs high, and a checkpoint is meant to survive a context compaction. Two data-surface defects made the mandate re-fire on the first prompt *after* a compaction: the canonical checkpoint file was rotated away at the compaction boundary, and a stale context-pressure reading survived it. The checkpoint now survives the boundary, and the pressure gauge resets at the boundary in a way that survives recomputation — so the safeguard fires when pressure is genuinely high, not because a compaction just happened.
- **Annotation keys in plan manifests now validate.** Underscore-prefixed keys (such as `_note`) used as human annotations inside a plan manifest failed schema validation, because the strict closed-world objects rejected every unknown key. The schema now allows underscore-prefixed annotation keys throughout while still rejecting unknown *real* fields — the closed-world guard is preserved. Alongside it, the three generators that refuse to regenerate a derived file when a master manifest fails schema now say so — a visible finding at session close plus a clear skip message on direct invocation — instead of silently leaving the derived file frozen at its last good state.
- **Installs no longer create a legacy hooks-state directory.** Fresh installs minted a `hooks/state/` directory under the install home even though runtime hooks state moved to the state root long ago. Installs stop creating it, the docs stop naming it, and new migration 0008 removes the empty leftover from existing installs at upgrade — a directory that still contains anything is preserved untouched.

## [v1.13.0]

Governance-integrity release — a large hardening train with four headline changes: ending a session now genuinely maintains the governance substrate (the backlog, indexes, and maintenance sweeps are written by one wired writer at session close, instead of silently recording work that never ran); the plan lifecycle simplifies to six statuses with a single terminal done-state; every retired or inert shipped surface is fully removed — from the shipped foundation *and* from existing installs at upgrade; and the idea-inbox becomes a real ledger that can answer "which captured ideas are fully settled and which are still owed an outcome." **This release ships three automatic upgrade-time migrations** — one normalizes your overlay-master's dimension-prefix lists to the array shape the union readers require, one converges your existing plans to the simplified status vocabulary, and one removes the retired surfaces from your install. All three run automatically during `install.sh --apply`, all are idempotent, and none needs anything from you by hand. See the [v1.13.0 release notes](docs/release-notes-v1.13.0.md).

### Added

- **The idea-inbox is now a ledger, not just a capture pile.** Every inbox item records its owning project and its disposition target in machine-readable form at capture, dispositions point at real resolvable targets, and a settled ledger plus an inbox index close the loop — so the backlog can answer "which captured ideas are fully settled and which are still open" instead of items quietly going stale.
- **The plan index groups plans by owning directory.** The cross-project plan index gains a by-project-directory section, so you can see at a glance which plans belong to which project or code tree.
- **Plans record when they finished.** Completing a plan now stamps a completion timestamp on its manifest, so "when did this actually close" is answerable from the record rather than from git archaeology.
- **An archive view without an archive status.** Archival is now a display-only view filter in the rendered indexes — old plans can be tucked out of sight without a status change that pretends to assert anything about their content.

### Changed

- **The plan status model shrinks to six statuses, and `completed` is the terminal done-state.** The statuses that asserted guarantees the system never delivered are retired; a plan now moves through `researching`, `planned`, `in-progress`, `paused`, and ends at `completed` (or `superseded` when another plan replaces it). An automatic upgrade-time migration converges every plan manifest in your existing corpus — including the status mirrors inside master plans — to the new vocabulary, mapping each retired token to `completed` in place. The rewrite is content-preserving — values, element order, and timestamps are preserved (a manifest that changes is re-serialized in the scaffolder's canonical formatting), with no re-derivation and no fresh timestamps on historical records. This corpus rewrite lands outside the installer's rollback envelope, but it is recoverable: each manifest that changes is snapshotted to a `.pre-0006` pre-image file beside it before it is rewritten, and because the mapping is convergent, re-running the upgrade re-converges the corpus.
- **Your overlay's dimension-prefix lists normalize to one shape.** An automatic upgrade-time migration converges the `dimension_prefixes` populations in your overlay-master to the canonical array-of-slugs shape the union readers require; an already-array leaf is left untouched, and the step is idempotent.
- **The shipped foundation carries zero dead services, and upgrades clean up after retirements.** Previously, a file removed from the shipped foundation would quietly linger in an existing install forever, because upgrades only visit files the new manifest lists. This release removes the retired surfaces from the ship, and an automatic upgrade-time migration removes them from your install by name — verifying each file against the shipped baseline first. A copy you have modified is never silently deleted: it is renamed aside with a `.foundation-retired` suffix so nothing of yours is lost. The migration also un-wires the one retired hook entry from your settings so it does not error on every session stop.
- **Session close is now the single writer of the governance substrate.** The work that keeps your plan corpus coherent — regenerating the backlog, refreshing indexes, running the due maintenance sweeps — is orchestrated by the session-close pipeline on a real cadence, with crash recovery that picks up an interrupted close, instead of being spread across writers that could silently skip.
- **Deterministic librarian capabilities no longer demand an interactive session.** The read-only, deterministic capabilities (index renders, validations, audits) now run on demand with no interactivity guard, while the two capabilities that actually apply changes refuse to run unattended unless an interactive-session signal is present — so automation can read and validate freely, and an accidental bare or automated invocation of a mutating capability is prevented.
- **Shipped documentation now states what each mechanism actually delivers.** A truth sweep across the shipped docs removed claims that oversold what gates and guards enforce — most notably, the read-only verification agents no longer advertise a functional-correctness pass they structurally cannot execute, and contract descriptions name the real consumer of each field.

### Fixed

- **Governance reads see your customizations.** Capabilities that read governance rules through the shared overlay merger resolve the union of the shipped foundation and your adopter overlay, converging readers that previously saw only the foundation copy. (A few low-level readers — the plans-root namespace guard and two derived-index renderers — still read the foundation copy directly.)
- **Hooks and capabilities survive a relocated Claude home.** Hard-coded home-directory assumptions in shipped hooks are gone; an install under a custom Claude home directory now resolves its own paths.
- **Crashes are no longer swallowed.** Shipped shell surfaces harden their error-exit behavior so a failing step surfaces as a failure instead of dying silently mid-pipeline, and hook crash evidence is preserved for diagnosis.
- **The upgrade-time migrations fail closed on missing Python.** Migrations 0006 and 0007 refuse with a remediation message rather than skipping silently when python3 is absent, and the installer warns at install time when the optional `pyyaml` module is missing. (Runtime capabilities keep their as-designed fail-open posture: when python3 is absent they skip their optional work silently rather than block a session.)
- **The traceability-matrix pipeline ships intact.** Through v1.12.x the ship transform stripped the `traceability-matrix` filename out of the shipped matrix code, so the pipeline read and wrote a hidden dotfile and the advertised auto-render could never fire for an adopter. The filename is preserved end-to-end now; the matrix renders to `traceability-matrix.md` as documented.
- **Plan-lifecycle caps are enforced correctly.** Five defects in how corpus-size and lifecycle caps were counted and applied are fixed, and the schema vocabulary around them is closed.
- **A train of precision fixes across the governance substrate.** The change-detection tier reports with per-surface precision; drift findings land in their correct namespace and placement; the capability registry's meta-gate validates what it claims to; the vault-writers ecosystem parser handles its real input shapes; the maintenance cadence model is wired to the scheduler infrastructure it names; and the memory-rules tier is governed like the rest of the substrate.
- **The plan index renders cleanly for the home spoke.** The catch-all home key no longer renders a useless directory annotation.
