...
# `cat` this replaces sleeps forever. THIS hook is the automatic tasks-render trigger
# (PostToolUse Edit|Write on every plan-tree manifest.json write), so a hang here
# stalls the whole plan-corpus derivation lane, silently, on every subsequent write.
# The timeout is on EVERY read and each line is accumulated as it arrives, so a stream
...
# still reads as empty. HOOKS_STDIN_WAIT overrides (whole seconds); a zero/non-numeric
# value falls back rather than reaching `read -t 0`, which on bash 3.2 arms no timer.
INPUT=""
if [ ! -t 0 ]; then
