...
#
# statusLine -> worker-statusline.sh. Emits a SINGLE status line describing the
# orchestrator/worker state (load-bearing + cosmetic). Claude Code renders
# stdout as the status line, so the body must print exactly one line and exit 0
# — even when there is no active worker.
#
