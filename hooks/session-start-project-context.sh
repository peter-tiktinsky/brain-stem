...
# `cat` this replaces sleeps forever and the SessionStart chain hangs with zero
# output. The timeout is on EVERY read and each line is accumulated as it arrives, so
# a stream that keeps delivering is never truncated and an expiry mid-stream keeps
# what already arrived; the trailing-newline trim reproduces `$(cat)` exactly, and
# absent input still reads as empty. HOOKS_STDIN_WAIT overrides (whole seconds); a
# zero/non-numeric value falls back rather than reaching `read -t 0`, which on bash
# 3.2 arms no timer at all.
INPUT=""
if [ ! -t 0 ]; then
