...
if not footer.strip():
    # ruled link grammar: same-directory relative markdown link — the overlap
    # matrix is a sibling of this index in the "Vault Writers" folder, so the
    # bare-basename target resolves from the index's own home (and stays
    # clickable in a renderer without a basename index)
    footer = "\n## See also\n\n- [_overlap-matrix](_overlap-matrix.md)\n"
