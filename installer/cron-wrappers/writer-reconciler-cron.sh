...
# ($CLAUDE_STATE_ROOT/vault-staging) and invokes
...
#
# RETIRES the legacy inbox-processor-cron.sh dead flags (--vault-root /
# --state-file → process.sh exit 2): this wrapper carries the reconciler's
# ACTUAL arg surface. Mirrors the librarian/architect cron-wrapper pattern
# (sources paths.sh + lockf.sh, sets PATH, logs under $CLAUDE_LOG_DIR,
# lock-protects the tick). The in-process lockf single-instance guard inside
# process.sh (HARD PREREQ) is the real concurrency boundary;
...
# The SINGLE canonical ephemeral staging root. paths.sh
